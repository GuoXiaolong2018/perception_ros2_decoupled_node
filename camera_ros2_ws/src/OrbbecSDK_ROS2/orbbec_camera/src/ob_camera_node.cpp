...
void OBCameraNode::onTemperatureUpdate(diagnostic_updater::DiagnosticStatusWrapper &status) {
  int error_code = 0;
  if (camera_name_ == "camera_head")
  {
    error_code = 0x10000;
  }
  else if (camera_name_ == "camera_breast")
  {
    error_code = 0x20000;
  }
  else if (camera_name_ == "camera_left_wrist")
  {
    error_code = 0x30000;
  }
  else if (camera_name_ == "camera_right_wrist")
  {
    error_code = 0x40000;
  }
  ...
  try {
    ...
    if (!is_running_.load() || !is_camera_node_initialized_.load()) {
      ...
      if (hardware_error_pub_) {
        ...
        msg->data = 0x200001 + error_code;
        hardware_error_pub_->publish(std::move(msg));
      }
      return;
    }
...
void OBCameraNode::setupPublishers() {
  ...
  hardware_error_pub_ = node_->create_publisher<std_msgs::msg::Int64>("/Base_HardwareError", 10);
}
