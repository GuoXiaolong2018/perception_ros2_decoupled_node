...
from launch.actions import DeclareLaunchArgument, ExecuteProcess, GroupAction, LogInfo, OpaqueFunction, TimerAction
...
def generate_launch_description():
    manage_rmw_zenohd_arg = DeclareLaunchArgument('manage_rmw_zenohd', default_value='true')
    args = [
        DeclareLaunchArgument('device_type', default_value='camera'),
        DeclareLaunchArgument('camera_name', default_value='camera_right_wrist'),
        DeclareLaunchArgument('depth_registration', default_value='false'),
        DeclareLaunchArgument('serial_number', default_value='CP93653000EB'),
    ...
    def create_launch_actions(context, args):
        node_actions = create_node_action(context, args)
        if LaunchConfiguration('manage_rmw_zenohd').perform(context).lower() != 'true':
            return node_actions
    ...
    return LaunchDescription(
        args + [
            manage_rmw_zenohd_arg,
            OpaqueFunction(function=lambda context: create_launch_actions(context, args))
        ]
    )
